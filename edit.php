<?php
require_once "./classes/classEdit.php";

if(isset($_POST['id']) && isset($_POST['newName']) && isset($_POST['newAge']) && isset($_POST['newEmail'])) {

    // Edit selected user
    $edit = new Edit($_POST['id'], $_POST['newName'], $_POST['newAge'], $_POST['newEmail']);
    $edit->connect();
    $edit->request();
    $edit->close();
}
?>

<!doctype html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-+0n0xVW2eSR5OomGNYDnhzAbDsOXxcvSN1TPprVMTNDbiYZCxYbOOl7+AMvyTG2x" crossorigin="anonymous">
    <link rel="stylesheet" href="./css/edit.css">
    <link rel="stylesheet" href="./css/style.css">
    <title>Редактирование</title>
</head>
<body>
<div class="header">
    <ul>
        <li><a href="index.php">Пользователи</a></li>
        <li><a href="add.php">Добавление нового пользователя</a></li>
        <li><a href="edit.php">Редактирование</a></li>
        <li><a href="delete.php">Удаление пользователя</a></li>
    </ul>
</div>
<form class="edit-form" action="edit.php" method="post">
    <div class="mb-3">
        <label class="form-label">ID пользователя, которого нужно отредактировать:</label>
        <input type="number" class="form-control" name="id">
    </div>
    <div class="mb-3">
        <label class="form-label">Новое имя:</label>
        <input type="text" class="form-control" name="newName">
    </div>
    <div class="mb-3">
        <label class="form-label">Возраст:</label>
        <input type="number" class="form-control" name="newAge">
    </div>
    <div class="mb-3">
        <label class="form-label">Новый email:</label>
        <input type="email" class="form-control" name="newEmail">
    </div>
    <button type="submit" class="btn btn-primary">Сохранить</button>
</form>
</body>
</html>
